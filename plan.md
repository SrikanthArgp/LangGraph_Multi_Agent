...
└── infra/                           # NEW (Phase 15–16): Terraform root — providers, S3+DynamoDB remote state, Lambda/API Gateway/CloudFront resources (Phase 15), ECS/ALB resources added in Phase 16
```
...
### Phase 15 — AWS Serverless Deployment (Lambda + API Gateway + CloudFront), Terraform via LocalStack first (Day 11–12)
...
### Phase 17 — CI Pipeline (Day 10, light)
Deliberately minimal — lint + fast tests on push, nothing more. Build/push/deploy automation now lives in **Phase 18 (CD: Lambda)** and **Phase 19 (CD: ECS Fargate)** rather than here, since both deploy targets exist by the time this phase is built — this phase stays scoped to lint+test only.
...
1. New IAM: a GitHub Actions deploy role, trust policy restricted to this repo + branch via the GitHub OIDC provider's condition keys (`token.actions.githubusercontent.com`) — **not** long-lived AWS access-key secrets stored in GitHub, matching how every other role in Phases 15/16 is narrowly scoped rather than broadly trusted
2. Workflow (`.github/workflows/cd-lambda.yml`), triggered on push to `main` after Phase 17's CI job passes: checkout → assume the deploy role via OIDC → build the Lambda-Web-Adapter image → push to ECR under a commit-sha tag
...
Same shape as Phase 18, targeting Phase 16's non-adapter image and task definition instead.
1. Reuse Phase 18's OIDC deploy role, with ECS-specific permissions added (`ecs:UpdateService`, `ecs:DescribeServices`) rather than provisioning a second role — revisit this if scoping later pushes toward splitting them, not resolved here
2. Workflow (`.github/workflows/cd-ecs.yml`), triggered the same way as Phase 18: build the non-adapter image, push to ECR under a commit-sha tag
...
## Cost Profile Summary (Phases 15–16)

| | Phase 15 (Serverless) | Phase 16 (ECS Fargate) |
|---|---|---|
