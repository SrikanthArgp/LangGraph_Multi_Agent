...
- **High availability.** Phase 17's ECS service runs at desired count 1 (called out there as a deliberate cost/availability tradeoff, not an oversight) — no multi-AZ redundancy, no auto-recovery beyond ECS restarting a crashed task, no multi-region. Phase 16's Lambda path is HA by nature of the platform, but Upstash/Supabase free tiers underneath it are still single-region.
- **CI/CD deploy automation.** Phase 15's pipeline (lint + fast tests on push) stops at green/red — no `terraform apply` stage, no automated rollback, no promotion between environments. Deploys to Phases 16–17 stay manual (`terraform apply` run by hand) even after Phase 15 exists.
