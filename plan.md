...
### Phase 15 — AWS Serverless Deployment (Lambda + API Gateway + CloudFront), Terraform via LocalStack first (Day 11–12) — Stage A complete (steps 1–5: Terraform scaffolding, Lambda Web Adapter, `boto3`, SSM-aware `config.py`, frontend static export); **Stage B complete and verified end-to-end against LocalStack (2026-07-08) — both backend compute (Lambda ×2/API Gateway/IAM/SSM) and frontend (S3/CloudFront/OAC)**; **Stage C (real AWS) CLOSED (2026-07-13)** — both LocalStack-only fidelity gaps flagged below (CloudFront Functions, `origin_read_timeout`) confirmed working correctly on real AWS, plus eleven further real gaps found getting `cd-lambda.yml` to actually deploy and work end-to-end: a missing Terraform binary on `ubuntu-latest`, several IAM permission gaps, a missing ECR repository policy (a real design gap, not just IAM scoping), and a genuine CloudFront-OAC-vs-app-JWT-auth architecture conflict fixed at the application level (custom `X-Auth-Token` header, since OAC overwrites `Authorization` en route to the streaming Lambda) — see `completed.md`'s Phase 15/18 Stage C entry and `real-aws-cicd-setup.md` for the full list
...
**Status (2026-07-13): Stages A/B/C all complete — Stage C (real AWS) CLOSED, verified end-to-end including a real browser test — see `completed.md`'s Phase 16/19 Stage C entry.** One design point below was decided differently once actually built, worth flagging here since it's a repo-structure decision, not just an execution detail: `infra/fargate/` was built **fully independent** of Phase 15's `infra/lambda-gate/` — its own ECR repository, own SSM `SecureString` parameters (at `/crag/prod-ecs/*`, not `/crag/prod/*`), own S3 frontend bucket, own scripts — rather than reusing Phase 15's ECR image/SSM parameters as step 6 below still describes. Reason: `infra/` was split into per-target Terraform roots (`infra/lambda-gate/`, `infra/fargate/`) specifically so each deploy target can be applied/destroyed/reasoned-about independently; a shared ECR repo or a cross-stack SSM read would have reintroduced exactly the coupling that split was meant to remove. Step 6 below is left as-is (it was the original design intent) rather than rewritten, since `completed.md`'s Phase 16 entry is the authoritative record of what was actually built and why it changed.
...
- **Planning note (2026-07-12): automating `infra/bootstrap` (the shared Terraform state bucket/lock table) into `cd.yml` itself, LocalStack-only, considered but not built.** Today it's a manual one-time `terraform apply`, run by hand whenever LocalStack gets reset — a real gap hit re-verifying Phase 18/19 end to end. Full design (a shared `bootstrap` job both deploy jobs `needs:`, gated on an AWS CLI existence check rather than Terraform state or a path filter, since `infra/bootstrap` has no remote backend of its own and its local state can't survive a CD job's `git clean -ffdx` checkout) lives in `cd-dispatcher-steps.md`'s matching "Deferred" section — not repeated here.

### Phase 18 — CD: Lambda (GitHub Actions) (Day 13, light) — built and verified end to end against both LocalStack and real AWS (2026-07-13); see `completed.md`'s Phase 15/18 Stage C entry and `real-aws-cicd-setup.md`
...
**Status (2026-07-16): built and live on real AWS** — `infra/eks/` + `gitops/multi-agent/`, cluster `crag-prod-eks` confirmed `Running`, CloudFront live at `https://d3m8imkry3vkn4.cloudfront.net`. See `completed.md`'s Phase 20 entry and `eks-enterprize-deploy-steps.md`'s "Built vs. Designed" section for exactly where the live build diverged from the design below (notably: direct IRSA+`boto3` secret access instead of the Secrets Store CSI Driver mentioned in step 5's cross-reference, one Terraform root instead of two for CloudFront, and HPA wired but not yet functional pending `metrics-server`). Phase 21 below is not built.
...
7. Validate on LocalStack Ultimate — the same trial activated 2026-07-08 for Phases 15–16, still active as of this writing (45-day window, lapses ~2026-08-22) — **flagged as the least-mature emulation of the three** — LocalStack's EKS support stands up a real local cluster and represents it through the mocked EKS API, rather than emulating a real AWS control plane the way Lambda/API Gateway do. Expect to hit undocumented gaps hands-on, the same way Phase 15 found CloudFront Functions silently not executing and a hardcoded 30s origin timeout — budget time for this rather than assuming `terraform apply` + `helm install` behaves identically to real EKS. If the trial has since lapsed by the time this phase is actually built, the $200 AWS promotional credit (see Funding note below) already covers validating directly against real AWS in a tight apply → test → destroy loop instead of paying for a LocalStack Base subscription just to test Terraform locally
8. **Deploy is manual, by hand — no CD automation covers this step, for either target.** `terraform apply` (cluster + node group + IRSA) against real AWS, then `helm install` the chart, both run directly rather than through a GitHub Actions workflow. This holds even after Phase 21 exists: Phase 21 only automates the *ongoing* image-tag bump into `gitops/multi-agent/values.yaml` (ArgoCD syncs that in-cluster) — the initial cluster bring-up and Helm install themselves are never wrapped in CI, unlike Phases 18–19's `cd-lambda.yml`/`cd-ecs.yml`, which do drive their target's actual deploy. Run the same manual smoke test as Phases 15–16 against the ALB/CloudFront URL
...
3. **The actual GitOps distinction from Phases 18–19, and the reason this phase exists:** `.github/workflows/cd-eks.yml` never touches the cluster directly. It builds and pushes the image to ECR under a commit-sha tag, then commits an updated image tag into `gitops/multi-agent/values.yaml` on `main` (a bot commit using the default `GITHUB_TOKEN` with `contents: write`, scoped to this repo only — no new PAT needed). ArgoCD's own reconciliation loop, already running in-cluster and watching that path, detects the Git diff and applies it; the GitHub Actions job ends at the commit, not at the deploy. **The image build/push itself stays inline in the workflow, the same as `cd-lambda.yml`/`cd-ecs.yml`** — `docker build`/`docker push` run directly as `cd-eks.yml` steps (ECR login → build → push), not delegated to a shell script. This matches those two workflows' own precedent: both reimplement `infra/lambda-gate/scripts/push_image.sh` / `infra/fargate/scripts/push_image.sh`'s logic inline rather than invoking the script, specifically so the CD workflow is the thing actually executing every step of the deploy, not a wrapper calling out to one — `push_image.sh` stays a local/manual-use script only, never something CI shells out to
...
**Update (2026-07-15):** That gate has cleared — Phases 15–19 are now fully built and verified end to end against both LocalStack and real AWS (both Stage Cs, Lambda and Fargate, closed 2026-07-13; see `completed.md`). Phase 20/21 are still design-only (nothing built yet) but are no longer blocked on anything else in this plan — ready to start whenever. Two other planning inputs are also now concrete rather than assumed: the LocalStack Ultimate trial activated 2026-07-08 for Phases 15–16 is still active (45-day window, lapses ~2026-08-22), and the $200 AWS promotional credit referenced in this section's Funding note is a real, already-available account, not a hypothetical funding plan.
