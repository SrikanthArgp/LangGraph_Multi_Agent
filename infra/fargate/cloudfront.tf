...
resource "aws_cloudfront_distribution" "this" {
  ...
  # api/main.py mounts /health deliberately outside the /v1 prefix, so it never matched the
  # /v1/* behavior above and fell through to the S3 frontend origin's default_cache_behavior —
  # a 404 NoSuchKey, not a real health-check failure. See infra/lambda-gate/cloudfront.tf's
  # matching comment for the full story — same gap, found by cd-ecs.yml's smoke check.
  ordered_cache_behavior {
    path_pattern             = "/health"
    target_origin_id         = "alb"
    viewer_protocol_policy   = "redirect-to-https"
