...
# state_lock_table_name outputs — run bootstrap's apply first. Shared bucket with
# infra/lambda-gate/, distinct key — see the comment on `key` below.
#
...
# version expects, and swap accordingly.
