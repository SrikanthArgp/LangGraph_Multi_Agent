...
# (infra/fargate/s3.tf's aws_s3_bucket.frontend — a separate bucket from
# infra/lambda-gate/s3.tf's, not shared). Mirrors push_image.sh's pattern for the backend image:
# Terraform provisions the bucket, this script is the separate manual/CI step that populates it.
#
...
#
# Usage: infra/fargate/scripts/sync_frontend.sh
set -euo pipefail
