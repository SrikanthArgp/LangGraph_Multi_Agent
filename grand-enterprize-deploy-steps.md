...
## Verified (2026-07-13): Stage C (real AWS) — CLOSED

`cd.yml` was dispatched with `target: fargate, environment: aws` against the real AWS account already used for Phase 15/18's Lambda Stage C (`completed.md`'s Phase 15/18 Stage C entry) — same account, same `cd-ecs-deploy-role`/OIDC provider/state bucket from `infra/bootstrap`, nothing new to provision at the bootstrap layer. Four real dispatch attempts were needed before a fully green run; full detail in `completed.md`'s new Phase 16/19 Stage C entry, summarized here:

1. **Same `paths-filter` single-commit-diff gap already documented for Lambda** (`ecr.tf`'s existing comment) — the triggering commit didn't touch `infra/fargate/**`, so the first dispatch took the fast (image-only) path against a stack that had never been applied, failing on a nonexistent ECR repo. Recovered the same way: a follow-up commit touching `infra/fargate/ecr.tf` forced the slow path.
2. **A real Terraform bug, not IAM**: `network.tf`'s `aws_security_group.ecs_task` description contained an apostrophe (`"...the ALB's security group..."`) — EC2's `CreateSecurityGroup` only accepts descriptions from a fixed charset (`a-zA-Z0-9. _-:/()#,@[]+=&;{}!$*`) that excludes apostrophes, rejected outright with `InvalidParameterValue`. Fixed by rewording the description.
3. **`cd-ecs-deploy-role` was missing `ecs:DeregisterTaskDefinition`** — only `RegisterTaskDefinition`/`DescribeTaskDefinition` were granted. The partial apply from gap 2's fix run had already registered a task definition; the next full apply needed to *replace* it (destroy+recreate), which calls `DeregisterTaskDefinition` on delete, not just on a plain `terraform destroy`. Fixed in `infra/bootstrap/github-oidc.tf`'s `cd_ecs_compute` policy, applied manually against the `real-aws` workspace (the one place applies happen by hand, since bootstrap provisions the very roles CD assumes). One real mistake made fixing this: the first `terraform plan` for the bootstrap fix omitted `-var="use_localstack=false" -var="aws_profile=crag-real-aws" -var="state_bucket_name=crag-terraform-state-247673029324"`, silently defaulting to LocalStack mode and hanging ~9 minutes retrying against an unreachable `localhost:4566` before being caught and killed — no state was affected, but worth noting as a real time cost, not a Terraform/AWS-side gap.
4. **Fourth dispatch went fully green**: full `terraform apply` created the VPC, ALB, ECS cluster/service, autoscaling target, CloudFront distribution, S3 frontend bucket, and all `/crag/prod-ecs/*` SSM parameters; smoke check (`/health` through CloudFront) passed; frontend built and synced.

**End state, confirmed working**: `https://d3j9oc5i8qv5ak.cloudfront.net` — backend `/health` → 200, frontend `/` → 200, both stacks' CloudFront domains manually smoke-tested side by side (Lambda: `https://d1at67obwaojws.cloudfront.net`). Neither stack's Terraform reads from or references the other at any layer (VPC, SSM path, ECR repo, S3 bucket, IAM role) — the independence goal from Deviation 1 above held under a real side-by-side dispatch, not just by code inspection.

**State left behind**: `infra/fargate/` is live on real AWS, not torn down — same "left running, cleanup is deliberate" posture as the Lambda stack (`completed.md`'s Phase 15/18 Stage C entry).
