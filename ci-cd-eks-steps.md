...
Status: **planning only, nothing built yet** (confirmed 2026-07-16: no `argocd` namespace, no `Application` resource in the live cluster, no `.github/workflows/cd-eks.yml`). **Its hard prerequisite is now satisfied**: [Phase 20 (EKS)](./eks-enterprize-deploy-steps.md) is applied and live on real AWS — a real cluster (`crag-prod-eks`) exists with a real `gitops/multi-agent/` Helm chart release (`helm install`, currently on image `:v1`) already running, so step 4 below has a real release to adopt rather than a hypothetical one. Remaining prerequisite: [the `cd.yml` dispatcher](./cd-dispatcher-steps.md) needs an `eks`/`all` option added to its `target` choice, since this workflow has no `push` trigger of its own anymore (design update, 2026-07-11 — see below). Also assumes [Phase 17's CI](./ci-pipeline-steps.md) is green on `main` before this workflow's trigger fires. **Note**: Phase 20's `gitops/multi-agent/` chart does not use the Secrets Store CSI Driver this doc's own reasoning was written assuming might exist alongside it — secrets are read directly via `backend_irsa` + `boto3` instead (see `eks-enterprize-deploy-steps.md`'s "Built vs. Designed" section) — this doesn't change anything in this doc's own design, since ArgoCD's job is only ever to sync `values.yaml`'s image tag, regardless of how the pod gets its secrets. Companion to [`enterprize-deploy-steps.md`](./enterprize-deploy-steps.md) (Phase 15), [`grand-enterprize-deploy-steps.md`](./grand-enterprize-deploy-steps.md) (Phase 16), [`eks-enterprize-deploy-steps.md`](./eks-enterprize-deploy-steps.md) (Phase 20), [`cd-lambda-deploy-steps.md`](./cd-lambda-deploy-steps.md) (Phase 18), [`cd-ecs-deploy-steps.md`](./cd-ecs-deploy-steps.md) (Phase 19), and [`cd-dispatcher-steps.md`](./cd-dispatcher-steps.md) — read the Phase 18/19 docs and the dispatcher doc first, since this doc only calls out where the GitOps shape *differs* from their direct-push pattern, not where it's the same.
...
4. **Adopt, don't duplicate, Phase 20's manually-installed Helm release.** Phase 20 Stage B step 12 already ran `helm install multi-agent gitops/multi-agent ...` by hand, before ArgoCD existed. Before step 3's `Application` does its first sync, confirm the release name/namespace it will manage (`multi-agent` / `multi-agent` namespace, per the manifest above) exactly matches what Phase 20 already installed — otherwise ArgoCD creates a *second*, competing set of resources rather than taking over the first. Simplest correct handoff: `helm uninstall multi-agent -n multi-agent` right before creating the `Application`, so ArgoCD's first sync is a clean create, not an adoption of unlabeled resources.
   ...
   **Real gap found doing this handoff (2026-07-16, from-scratch redeploy):** `helm uninstall` deletes the `Ingress` object, and the AWS Load Balancer Controller deprovisions its backing ALB when it does — it does **not** just detach and wait. When ArgoCD's first sync then recreates the `Ingress` fresh, the controller provisions a **brand-new physical ALB with a different DNS name**, not the same one Stage 6 already pointed `infra/eks/cloudfront.tf`'s `data "aws_lb"` lookup at. Since that lookup only re-resolves on a `terraform apply`, CloudFront keeps serving from the now-deleted old ALB's DNS name until one runs — the symptom is a `502` from CloudFront immediately after ArgoCD reports `Synced`/`Healthy`, even though the backend pod and target group are both actually fine. Fix: run `terraform apply -var="use_localstack=false"` in `infra/eks` (no targets needed — the `data "aws_lb"` source is what needs re-reading) right after the handoff, before declaring it done. Confirmed via `aws elbv2 describe-load-balancers` showing only the new ALB's DNS name (old one gone, `CreatedTime` matching the handoff) while `aws cloudfront get-distribution`'s origin still listed the old, no-longer-existent name.
...
6. **Build and tag** the non-adapter image (same `Dockerfile.ecs` / plain `CMD ["python", "run_api.py"]` build Phase 16 already established), **checked out at `inputs.sha`** (the exact commit `ci.yml` validated — see `cd-dispatcher-steps.md`'s sha-propagation note), with an immutable `eks-<sha>` tag, push to ECR — a distinct tag prefix from Phase 16's `ecs-<sha>` and Phase 20's own build-path tag, so all three deploy targets can be rolled back independently without fighting over one mutable tag, matching `eks-enterprize-deploy-steps.md`'s own `:eks` tagging note. **The build/push itself runs as inline `docker build`/`docker push` steps in this workflow (see the YAML below), the same as `cd-lambda.yml`/`cd-ecs.yml` — not delegated to `infra/lambda-gate/scripts/push_image.sh` or `infra/fargate/scripts/push_image.sh`.** Those scripts stay local/manual-use only (e.g. Phase 20's own Stage A step 1 initial push); the CD workflow reimplements the same three commands itself so it's the thing actually executing every step of the deploy, not a wrapper calling out to one.
